...
def string_list_please() -> list:
    ...
    list_string = ['l', 'love', 'you']
    return list_string


def dictionary_please() -> dict:
    """Returns a dictionary, anything you like."""
    dict_like = {'jack':85, 'jenny':90}
    return dict_like
...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    no_fire = list(input_string.upper())
    fire = '🔥'.join(no_fire)
    fires = '🔥' + fire + '🔥'
...
def best_letter_for_pets() -> str:
    ...
    for i in the_alphabet:
        if the_count < len(pet_filter(i)):

            the_count = len(pet_filter(i))
